...
namespace SupremeFiction.UI.SupremeRulerModdingTool.WinForm.HotKey
{
    ...
    /// </summary>
    public sealed class HotKeyManager : Component, IMessageFilter, IDisposable
        // , IEnumerable, IEnumerable<GlobalHotKey>, IEnumerable<LocalHotKey>, IEnumerable<ChordHotKey>
    {
        ...
        //Will keep track of all the registered GlobalHotKeys

        private static bool InChordMode; //Will determine if a chord has started.
        private readonly List<ChordHotKey> ChordHotKeyContainer = new List<ChordHotKey>(); //Will hold our ChordHotKeys.
        private readonly IntPtr FormHandle; //Will hold the handle of the form.

        private readonly List<GlobalHotKey> GlobalHotKeyContainer = new List<GlobalHotKey>();
        //Will hold our GlobalHotKeys

        private readonly List<LocalHotKey> LocalHotKeyContainer = new List<LocalHotKey>(); //Will hold our LocalHotKeys.

        //Keep the previous key and modifier that started a chord.
        private Keys PreChordKey;
        private Modifiers PreChordModifier;
        ...
        {
            get { return (Form) Control.FromHandle(FormHandle); }
        }
        ...
        {
            get { return LocalHotKeyCount + GlobalHotKeyCount + ChordHotKeyCount; }
        }
        ...
        /// </summary>
        public IEnumerable EnumerateGlobalHotKeys
        {
            get { return GlobalHotKeyContainer; }
        }

        /// <summary>
        ///     Use for enumerating through all LocalHotKeys.
        /// </summary>
        public IEnumerable EnumerateLocalHotKeys
        {
            get { return LocalHotKeyContainer; }
        }

        /// <summary>
        ///     Use for enumerating through all ChordHotKeys.
        /// </summary>
        public IEnumerable EnumerateChordHotKeys
        {
            get { return ChordHotKeyContainer; }
        }
        ...
        private void GlobalHotKeyPropertyChanged(object sender, PropertyChangedEventArgs e)
        {
                ...
                if (error != 0 && error != 2)
                    if (!SuppressException)
                    {
                        throw new HotKeyUnregistrationFailedException("The hotkey could not be unregistered",
                            GlobalHotKeyContainer[id], new Win32Exception(error));
                    }
            }
        }
        ...
        /// <exception cref="HotKeyAlreadyRegisteredException">
        ///     Thrown is a GlobalHotkey with the same name, and or key and modifier
        ...
        /// <param name="hotKey">The hotKey which will be added. Must not be null and can be registered only once.</param>
        /// <exception cref="HotKeyAlreadyRegisteredException">
        ///     thrown if a LocalHotkey with the same name and or key and modifier
        ///     has already been added.
        /// </exception>
        public bool AddLocalHotKey(LocalHotKey hotKey)
        {
            ...
            bool ChordExits = ChordHotKeyContainer.Exists
                (
                    delegate(ChordHotKey f) { return (f.BaseKey == hotKey.Key && f.BaseModifier == hotKey.Modifier); }
                );
        ...
        public bool AddChordHotKey(ChordHotKey hotKey)
        {
            ...
            bool LocalExists = LocalHotKeyContainer.Exists
                (
                    delegate(LocalHotKey f) { return (f.Key == hotKey.BaseKey && f.Modifier == hotKey.BaseModifier); }
                );
        ...
        {
            if (GlobalHotKeyContainer.Remove(hotKey))
            {
                --GlobalHotKeyCount;
        ...
        {
            if (LocalHotKeyContainer.Remove(hotKey))
            {
                --LocalHotKeyCount;
                return true;
            }
        ...
        {
            if (ChordHotKeyContainer.Remove(hotKey))
            {
                --ChordHotKeyCount;
                return true;
            }
        ...
        public bool RemoveHotKey(string name)
        {
            LocalHotKey local = LocalHotKeyContainer.Find
                (
                    delegate(LocalHotKey l) { return (l.Name == name); }
                );
            ...
            ChordHotKey chord = ChordHotKeyContainer.Find
                (
                    delegate(ChordHotKey c) { return (c.Name == name); }
                );
            ...
            GlobalHotKey global = GlobalHotKeyContainer.Find
                (
                    delegate(GlobalHotKey g) { return (g.Name == name); }
                );
        ...
        public bool HotKeyExists(ChordHotKey chordhotkey)
        {
            return ChordHotKeyContainer.Exists
                (
                    delegate(ChordHotKey c) { return (c == chordhotkey); }
                );
        }
        ...
        {
            var Key = (Keys) HotKeyShared.ParseShortcut(shortcut).GetValue(1);
            var Modifier = (Modifiers) HotKeyShared.ParseShortcut(shortcut).GetValue(0);
            switch (ToCheck)
            {
                case CheckKey.GlobalHotKey:
                    return GlobalHotKeyContainer.Exists
                        (
                            delegate(GlobalHotKey g) { return (g.Key == Key && g.Modifier == Modifier); }
                        );

                case CheckKey.LocalHotKey:
                    return (LocalHotKeyContainer.Exists
                        (
                            delegate(LocalHotKey l) { return (l.Key == Key && l.Modifier == Modifier); }
                        )
                            |
                            ChordHotKeyContainer.Exists
                                (
                                    delegate(ChordHotKey c) { return (c.BaseKey == Key && c.BaseModifier == Modifier); }));
            ...
            switch ((KeyboardMessages) m.Msg)
            {
                ...
                case (KeyboardMessages.WmKeydown):
                    ...
                    if (InChordMode)
                    {
                        ...
                        ChordHotKey ChordMain = ChordHotKeyContainer.Find
                            (
                                ...
                                {
                                    return ((cm.BaseKey == PreChordKey) && (cm.BaseModifier == PreChordModifier) &&
                        ...
                        InChordMode = false;
                        new Computer().Audio.PlaySystemSound(SystemSounds.Exclamation);
                        return true;
                    }
                    ...
                    if (ChordBase != null)
                    {
                        ...
                        var e =
                            new PreChordHotKeyEventArgs(new LocalHotKey(ChordBase.Name, ChordBase.BaseModifier,
                                ChordBase.BaseKey));
                ...
                case (KeyboardMessages.WmHotKey):
                    ...
                    GlobalHotKey Pressed = GlobalHotKeyContainer.Find
                        (
                            delegate(GlobalHotKey g) { return ((g.Id == Id)); }
                        );
