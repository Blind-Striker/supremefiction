...
namespace SupremeFiction.UI.SupremeRulerModdingTool.Core.Presenters
{
    internal class UnitEditorPresenter : BasePresenter<IUnitEditorView, IUnitEditorPresenter>, IUnitEditorPresenter
    {
        ...
        private readonly string _defaultUnitFilePath;
        ...
        public void PrepForUpdate(string path)
        {
            Name = Path.GetFileNameWithoutExtension(path);
            UnitPath = path;
            InitializeNewView(Name);

            _preventEventFire = true;

            _fileHelper.Init(path);

            SetDataTableRows(_categories);
            FillGrid();
        ...
        public void Save()
        {
            ...
            const string Extension = ".UNIT";
            string directoryName = _dataFolder;

            string newFileName = !UnitPath.IsNullOrEmpty() 
                ? string.Format("{0}-{1}{2}", fileName, DateTime.Now.ToString("yyyyMdhhmmss"), Extension) 
                : string.Format("{0}{1}", fileName, Extension);

            string newFilePath = Path.Combine(directoryName, newFileName);

            string readFile = IoHelper.ReadFile(_defaultUnitFilePath);
        ...
        public void InitializeNewView(string name)
        {
            _preventEventFire = true;

            SetDataTables(_categories);
            _fileHelper.Container.Clear();
            _isDirty = false;
            Name = name;
            UnitTabPage.TabName = Name;

            IEnumerable<ComboboxItem> categoryList = _categories.ToComboboxItems();
        ...
        {
            if (_unitEditorViewModel.SelectedRows.IsNullOrEmpty())
            {
                return;
            }
        ...
        private void SetDataTables(IEnumerable<string> categories)
        {
            ...
            foreach (var category in categories)
            {
                ...
                List<string> columnsByCategory = _fileHelper.GetColumnsByCategory(category);
        ...
        private void SetDataTableRows(IEnumerable<string> categories)
        {
            foreach (var category in categories)
            {
                DataTable dataTable = _dataTablesByCategory[category];

                IEnumerable<ItemModel> items = _fileHelper.GetItemModels(category, null, null, null);
